...
module.exports = function(el, bindings, state) {
  try {
    ...
    re.addEventListener('keyup', wrapped(onoff(kb, false)))
    re.addEventListener('keydown', wrapped(onoff(kb, true)))
    ee.addEventListener('mouseup', wrapped(onoff(mouse, false)))
    ee.addEventListener('mousedown', wrapped(onoff(mouse, true)))
