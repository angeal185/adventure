...
SpatialEventEmitter.prototype = {
  size: 16,
  on(event, bbox, listener) {
  ...
  emit(event, bbox /*, ...args */ ) {
    ...
    let list = this.infinites[event].slice();
  ...
  create_root(bbox) {
    ...
    size = self.size,
    base = [
      Math.floor(bbox.x0() / size) * size, Math.floor(bbox.y0() / size) * size, Math.floor(bbox.z0() / size) * size
    ],
...
function finite(bbox) {
  return isFinite(bbox.x0()) && isFinite(bbox.x1()) && isFinite(bbox.y0()) &&
    isFinite(bbox.y1()) && isFinite(bbox.z0()) && isFinite(bbox.z1())
}
